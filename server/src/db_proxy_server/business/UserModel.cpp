    ...
    if (pDBConn)
    {
        ...
        if(pResultSet)
        {
            while (pResultSet->Next())
            {
                ...
                cUser.set_password(pResultSet->GetString("password"));
                cUser.set_salt(pResultSet->GetString("salt"));
...
bool CUserModel::insertUser(DBUserInfo_t &cUser)
{
    ...
    if (pDBConn)
    {
        string strSql = "insert into IMUser(`id`,`sex`,`nick`,`password`,`domain`,`name`,`phone`,`email`,`avatar`,`sign_info`,`departId`,`status`,`created`,`updated`) values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
